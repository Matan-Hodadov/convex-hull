...
class Facet:
    ...
    def __init__(self, p0, p1, p2, edges_dict):  # TODO need to check maybe to order p0 p1 p2 clockwise!
...
def define_order(p1, p2, p3, points_till_now_dict):
    ...
    for p in points_till_now_dict:
        if not points_equal(p1, points_till_now_dict[p]) and not points_equal(p2, points_till_now_dict[p]) \
                and not points_equal(p3, points_till_now_dict[p]):
    ...
    for comb in combinations:
        ...
        m = [[1, 1, 1, 1], [p1_temp.x, p2_temp.x, p3_temp.x, d.x], [p1_temp.y, p2_temp.y, p3_temp.y, d.y],
             [p1_temp.z, p2_temp.z, p3_temp.z, d.z]]
        det = np.linalg.det(m)
        if det > 0:
            c = comb
            break
